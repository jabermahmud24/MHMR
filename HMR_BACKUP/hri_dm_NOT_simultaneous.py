...
def do_simulation(cx, cy, cz, cyaw, cpitch, croll, world_ref_traj):
...
def main():
    ...
    while np.linalg.norm(robot_position - B_pos) > 0.1 and step < max_steps:
        ...
        cz_value = random.uniform(0.7, 0.8)

        # Collect the trajectory points
        cx.append(robot_position[0])
        cy.append(robot_position[1])
        cz.append(cz_value)
        # cz.append(0.7)  # Default z value
        cyaw.append(0.0)
        cpitch.append(0.0)
        croll.append(0.0)
    ...
    # Now that we have the trajectory, use the robot control code to simulate the robots following it
    cx = np.array(cx)
    cy = np.array(cy)
    cz = np.array(cz)
    cyaw = np.array(cyaw)
    ...
    x1, y1, z1, yaw1, pitch1, roll1, x2, y2, z2, yaw2, pitch2, roll2 = do_simulation(
        cx, cy, cz, cyaw, cpitch, croll, world_ref_traj)
