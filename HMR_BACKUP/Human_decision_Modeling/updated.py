...
class State:
    def __init__(self, desired_position, world_ref_traj, dh_params):
        self.dh_params = dh_params
        self.x_base = 2.5
        self.y_base = 1.9
        self.yaw_base = 0
        self.theta = np.zeros(8)  # Initialize joint angles
        ...
        # Compute initial theta using inverse kinematics
        # xyz = np.array([desired_position[0] - self.x_base,
        #                 desired_position[1] - self.y_base,
        #                 world_ref_traj[0, 2]])
        # abc = world_ref_traj[0, 3:6]
        robot = RobotSerial(self.dh_params)
        end = Frame.from_euler_3(abc, xyz)
        robot.inverse(end)
        self.theta = robot.axis_values.copy()
        self.update_pose(robot.forward(self.theta))

    def update_pose(self, f):
        self.x_body, self.y_body, self.z_body = f.t_3_1.flatten()
        self.roll_body, self.pitch_body, self.yaw_body = f.euler_3
        self.x_world = self.x_base + self.x_body
        self.y_world = self.y_base + self.y_body
        self.z_world = self.z_body
        self.yaw_world = self.yaw_body
        self.pitch_world = self.pitch_body
        self.roll_world = self.roll_body
...
def update(state, ustar):
    state.theta += dt * ustar[1:].flatten()
    state.yaw_base += ustar[0] * dt
    state.yaw_base = (state.yaw_base + np.pi) % (2 * np.pi) - np.pi
    state.x_base += ustar[0] * dt * np.cos(state.yaw_base)
    state.y_base += ustar[0] * dt * np.sin(state.yaw_base)
    # Update the robot's forward kinematics
    robot = RobotSerial(state.dh_params)
    f = robot.forward(state.theta)
    # Update the state's world and body coordinates
    state.update_pose(f)
...
while np.linalg.norm(robot_position - B) > 0.1 and step < max_steps:
    ...
    D = np.array([[0, 0, 0], 
        ...
        [0,0,0]])
    if step == 0:
        state = State(desired_position=desired_position, world_ref_traj=world_ref_traj, dh_params=dh_params)
        theta = state.theta
    ...
    state, ee_pose = update(state, ustar)    
