...
# A = np.array([0.0, 0.0])   # Starting point A
...
class State:
    ...
    def __init__(self,world_ref_traj):   
        ...
        model_path='nadam_cvae_model_without_initial_theta.pth',
        urdf_path='fetch.urdf',
        n_samples=n_samples)

        self.theta = new_joint_angles
        print('inverse kinemtics theta:',self.theta)
        ...
        self.theta = [0, 0, -0.02127847287413969 ,
                            0.0 ,0.0006472529125291289 ,0.0 ,0.02065431601470017, 0]
        ...
        # self.x_world = self.x_base + ee_pos[0]*math.cos(self.yaw_base) - ee_pos[1]*math.sin(self.yaw_base)
        # self.y_world = self.y_base + ee_pos[0]*math.sin(self.yaw_base) + ee_pos[1]*math.cos(self.yaw_base)
...
def get_B(state, joint_angle_combination):
    """
    Compute the B matrix and forward kinematics using joint angles.

    Parameters:
    - dh_params: Denavit-Hartenberg parameters for the robot.
    - state: An object containing the current state (e.g., state.yaw_base).
    - joint_angle_combination (list or array): Joint angles for the robot.
    - dt (float): Time step for integration.

    Returns:
    - B (numpy.ndarray): The computed B matrix (6x9).
    - f (Any): Forward kinematics result from RobotSerial.
    """
    # Initialize RobotSerial and compute forward kinematics
    # robot = RobotSerial(dh_params)
    # theta = joint_angle_combination
    # f = robot.forward(theta)

    # Compute Jacobian using the refactored test_jacobian.py
    jacobian = compute_jacobian(joint_angle_combination)
    print(jacobian)

    # if jacobian is None:
    #     raise ValueError("Failed to compute Jacobian using test_jacobian.py.")

    # Initialize B matrix
    B = np.zeros((6, 9))

    # Assign the Jacobian scaled by dt to columns 1 to 8 (indices 1 to 8)
    B[:, 1:9] = jacobian * dt
    # B[:, 1:9] = jacobian * dt

    # Assign the first column based on the base yaw angle
    B[0, 0] = dt * math.cos(state.yaw_base)
    B[1, 0] = dt * math.sin(state.yaw_base)

    # B[0, 1] = - dt * math.sin(state.yaw_base)
    # B[1, 1] = dt * math.cos(state.yaw_base)
...
def update(state, ustar, B, joint_angle_combination):  
    ...
    # state.x_base += (ustar[0] * dt * math.cos(state.yaw_base)) - (ustar[1] * dt * math.sin(state.yaw_base))
    # state.x_base = float(state.x_base)
    # state.y_base += (ustar[0] * dt * math.sin(state.yaw_base)) + (ustar[1] * dt * math.cos(state.yaw_base))
    ...
    # state.x_world = state.x_base + ee_pos[0]*math.cos(state.yaw_base) - ee_pos[1]*math.sin(state.yaw_base)
    # state.y_world = state.y_base + ee_pos[0]*math.sin(state.yaw_base) + ee_pos[1]*math.cos(state.yaw_base)
