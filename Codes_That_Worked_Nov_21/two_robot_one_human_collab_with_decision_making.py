...
phi_k_init = 0.5      # Human preference parameter
phi_k = phi_k_init
...
def pos_to_idx(pos):
    idx_x = np.argmin(np.abs(x_range - pos[0]))
    idx_y = np.argmin(np.abs(y_range - pos[1]))
    return (idx_x, idx_y)
...
def upsample_trajectory(world_ref_traj, target_num_points, kind='cubic'):
    """
    Upsamples the trajectory to the desired number of points.

    Parameters:
    - world_ref_traj: np.ndarray of shape (6, N)
    - target_num_points: int, desired number of points M
    - kind: str, type of interpolation ('linear', 'cubic', etc.)

    Returns:
    - upsampled_traj: np.ndarray of shape (6, M)
    """
    original_num_points = world_ref_traj.shape[1]
    if target_num_points < original_num_points:
        raise ValueError("Target number of points must be greater than the original number of points.")

    # Original parameter (e.g., normalized time)
    t_original = np.linspace(0, 1, original_num_points)
    # Target parameter
    t_target = np.linspace(0, 1, target_num_points)

    # Initialize the upsampled trajectory
    upsampled_traj = np.zeros((6, target_num_points))

    # Interpolate each row of the trajectory
    for i in range(6):
...
def main():
    ...
    # Simulation loop
    while np.linalg.norm(robot_position - B_pos) > 0.3 and step < max_steps:
        cnt += 1
        ...
        if len(path_coords_robot) > 1:
            ...
            next_pos_robot = robot_position.copy()  # Stay in place if no path found

        if len(path_coords_human) > 1:
            next_pos_human = path_coords_human[1]
        else:
            next_pos_human = robot_position.copy()  # Stay in place if no path found
        ...
        #             idx_new = pos_to_idx(new_position)
        #             if obstacle_map[idx_new] == 0:
        #                 robot_position = new_position
        #                 robot_positions[-1] = robot_position.copy()
        #                 stuck_count += 1
        #                 print("stuck count:", stuck_count)  
        #                 print(f"Step {step}: Robot was stuck. Moved a small step towards current heading angle.")
        #             else:
        #                 print(f"Step {step}: Cannot move towards current heading due to obstacle.")
        #         else:
        #             print(f"Step {step}: Cannot move towards current heading due to out-of-bounds.")
        # else:
        #     # Initialize heading angle on the first step based on movement
        #     movement_vector = robot_positions[-1] - robot_positions[-2]
        #     if np.linalg.norm(movement_vector) != 0:
        #         heading_angle = math.atan2(movement_vector[1], movement_vector[0])
        ...
        # cx2 = np.linspace(0, 1, 209)
        # f = interp1d(cx1, cx)
        # cx = f(cx2)

        # cy1 = np.linspace(0, 1, len(cy))
        # cy2 = np.linspace(0, 1, 209)
        # f = interp1d(cy1, cy)
        # cy = f(cy2)

        cz = np.random.uniform(0.7, 0.7, len(cy))
        ...
        # world_ref_traj = upsample_trajectory(world_ref_traj, target_num_points, kind='cubic')


        # print(len(world_ref_traj[0,:]))
        ...
        if show_animation:
            ...
            ax2.plot(x1[-1], y1[-1], "ob", label="Robot 1 Trajectory")
            ax2.plot(x2[-1], y2[-1], "og", label="Robot 2 Trajectory")
    ...
    print("total stuck count:", stuck_count)
